...
function generateMockDeck(testCase) {
    ...
    if (Array.isArray(testCase)) {
        ...
        const playerIds = Object.keys(testCase).map(Number).sort((a, b) => a - b);
        ...
        const maxHandSize = Math.max(...Object.values(testCase).map(hand => hand.length));
    ...
    if (invalidTestCards.length > 0) {
        throw new Error(`Test cards not in standard deck: ${invalidTestCards.join(', ')}`);
    }
